@extends('layouts.app') @section('title') Arrival Data @endsection @section('content') @include('components.sidebar')
    ...
    <div id="crud-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
      <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
          ...
          <!-- Modal body -->
          <form class="p-4 md:p-5" action="{{ route('arrival.tambah')}}" method="post">
            @csrf
            ...
            <div class="grid gap-4 mb-4 grid-cols-2">
              ...
              <div class="col-span-2 sm:col-span-1">
                <label for="waktu" class="label-jadwal">Jam Kedatangan</label>
                <input type="time" name="waktu" id="waktu" class="input-jadwal" required="" />
              </div>
              <div class="col-span-2">
                <label for="dari" class="label-jadwal">Dari</label>
                <input type="text" name="dari" id="dari" class="input-jadwal" required="" />
              </div>
            </div>
        ...
        <tbody class="">
          ...
          <tr class="bg-white border-b">
            ...
            <td class="px-6 py-4 flex space-x-5 items-center justify-center">
              <form action="{{route('arrival.hapus', $data['id'])}}" method="post" onsubmit="return confirm ('hapus jadwal penerbangan {{ $data['id_penerbangan']}} ?')">
                @csrf @method('DELETE')
              ...
              >
               ...
               <a
                id="edit-data"
                href="{{ route('arrival.edit', $data->id) }}"
                class="editData block bg-gray-500 text-gray-50 hover:bg-gray-400 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-3.5 py-2 text-center"
              >
                Edit
              </a>
          ...
          <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <form class="p-4 md:p-5">
              ...
              <div class="grid gap-4 mb-4 grid-cols-2">
                ...
                <div class="col-span-2 sm:col-span-1">
                  <label for="update-waktu" class="block mb-2 text-sm font-medium text-gray-900">Jam Kedatangan</label>
                ...
                <div class="col-span-2">
                  <label for="update-dari" class="block mb-2 text-sm font-medium text-gray-900">Dari</label>
                  <input type="text" name="dari" id="update-dari" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" required />
                </div>
              </div>
...
<script>
  ...
  $('body').on('click', '#edit-data', function () {
    ...
    let arrivalId = $(this).data('id');

    $.ajax({
        url: `data/${arrivalId}/edit`,
        type: 'GET',
        cache: false,
        success:function(response){

          $('#arrival-id').val(response.data.id);
          ...
          $('#update-dari').val(response.data.dari);
